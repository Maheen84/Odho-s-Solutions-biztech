...
// Ensure ChevronDown, Plus, Minus are imported from lucide-react if not already
if (!content.includes('ChevronDown')) {
  content = content.replace(
    'Eye \n} from "lucide-react";',
    'Eye, ChevronDown, Plus, Minus \n} from "lucide-react";'
  );
}

// Find the start of the SEO section
const seoStartMarker = '{isSeo && (';

let startIndex = content.indexOf(seoStartMarker);
if (startIndex === -1) {
  console.log('Could not find {isSeo && (');
  process.exit(1);
}

// Find the end of the SEO section (before COMMON BOTTOM SECTIONS)
const endMarker = '{/* COMMON BOTTOM SECTIONS: 6 SERVICES CARDS & BEST PROJECTS */}';
...
// Back up a bit to find the closing )} of the isSeo block
...
const absoluteEndIndex = startIndex + lastBraceIndex + 2;

const newSeoContent = `{isSeo && (
        <>
          {/* Hero & Intro */}
                ...
                <div className="lg:col-span-6">
                  ...
                  <div className="bg-[#fafafa] border border-[#eee] rounded-2xl p-8 mb-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div>
                        <div className="text-3xl font-black text-primary mb-1">+210% Organic Leads</div>
                        <p className="text-xs text-muted font-medium">Proven client growth metrics within 6 months.</p>
                      </div>
                      <div>
                        <div className="text-3xl font-black text-primary mb-1">-42% Average CAC</div>
...
const newFileContent = content.substring(0, startIndex) + newSeoContent + content.substring(absoluteEndIndex);
fs.writeFileSync('src/pages/ServiceDetail.tsx', newFileContent);
console.log('Successfully replaced SEO content in ServiceDetail.tsx');
