            ...
            {projects.map((project, idx) => (
              <article key={idx} className="bg-white border border-line rounded-[32px] overflow-hidden group hover:shadow-custom hover:border-primary/20 transition-all duration-300 flex flex-col">
                ...
                <div className="p-8 flex flex-col flex-grow">
                   ...
                   <NavLink to={`/services/${project.slug}`} className="btn btn-dark w-full gap-2 mt-auto">View Details <ArrowUpRight size={18} /></NavLink>
                </div>
              </article>
            ))}
          </div>
        ...
        <div className="container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {showcase.map((item, idx) => (
              <div 
                key={idx} 
                className="group relative h-[450px] md:h-[500px] rounded-[40px] overflow-hidden shadow-lg border border-line cursor-pointer"
              >
                {/* Background Image */}
                <img 
                  src={item.img} 
